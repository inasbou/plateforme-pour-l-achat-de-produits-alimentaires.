...
const Stores = () => {
  ...
  useEffect(() => {
    const prices = storesData[0].products.map((product) => product.price);
  ...
  const getUniqueItems = (key) => {
    return [...new Set(storesData[0].products.map((product) => product[key]))];
  };

  const filteredProducts = storesData[0].products.filter((product) => {
  ...
  const uniquenames = [
    ...new Set(storesData[0].products.map((product) => product.name)),
  ];
  const uniquebrands = [
    ...new Set(storesData[0].products.map((product) => product.brand)),
  ];
  const uniquetypes = [
    ...new Set(storesData[0].products.map((product) => product.type)),
  ];
  ...
  return (
    ...
    <div className="flex flex-col   w-full justify-center">
      ...
      <div className="flex flex-col  md:flex-row bg-green-600 justify-center">
        <h1 className=" hidden text-2xl py-4 font-bold text-white mx-6 md:flex flex-row "> <MdOutlineFilterList className="text-white" />Filter your result </h1>
